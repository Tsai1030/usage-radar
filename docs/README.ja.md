...
[最新リリース](https://github.com/Tsai1030/usage-radar/releases/latest) からビルド済みインストーラーをダウンロードしてください。

1. `.msi` または NSIS `.exe`(Windows)を取得。macOS / Linux 用は後続リリースで提供予定。それまでは下の開発者向け手順を。
2. ダブルクリックでインストール。Windows SmartScreen が*不明な発行元*の警告を出します(コード署名は v0.3 で対応予定) → **詳細情報 → 実行** をクリック。
3. スタートメニューから **Usage Radar** を起動。
